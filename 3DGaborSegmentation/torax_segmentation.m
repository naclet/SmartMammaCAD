...
stack_allm=zeros(size(stack_all));
stack_allm(:,:,chest_midpoint:end-endslice,:)=stack_all(:,:,chest_midpoint:end-endslice,:);
if nargin>2
    % apply the same processing to other images
    for i=1:nargin-2
        stp=varargin{i};
        if strcmp(database,'NME')
        stp=flip(permute(stp,[1 3 2]),2);
        end
        stpm=zeros(size(stp));
        stpm(:,chest_midpoint:end-endslice,:)=stp(:,chest_midpoint:end-endslice,:);
        varargout{i}=stpm;
    end
end
end